...
link=$(sudo curl -Ls "https://api.github.com/repos/geeksmax/SSH-Pro/releases/latest" | grep '"browser_download_url":' | sed -E 's/.*"([^"]+)".*/\1/')
...
bash <(curl -Ls https://raw.githubusercontent.com/geeksmax/SSH-Pro/main/Requirements/nethogs.sh --ipv4)
fi
